...
/**
 * 사이드바 + 모바일 BottomSheet 양쪽에서 공유하는 필터 폼 컨트롤.
 * (응원팀 / 기간 / 예산 / 인원 / 이동수단 / 코스 생성 / 시연 모드)
 *
 * 부모는 layout shell (aside 또는 sheet body) 만 결정 — 본 컴포넌트는 form 만 담당.
 */
...
interface FilterControlsProps {
  /** sheet 안에서 사용 시 코스 생성 후 sheet 닫기 */
  closeSheetOnGenerate?: boolean;
}

export function FilterControls({ closeSheetOnGenerate = false }: FilterControlsProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const closeSheet = useMobileSheet((s) => s.close);
  ...
  return (
    <>
          ...
          {PARTY.map((o) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex items-center justify-center gap-1 rounded-xl border px-2 py-2 text-xs font-bold transition-colors",
                  active
          ...
          {TRANSPORT.map((o) => {
            ...
            return (
              <button
                ...
                title={o.label}
                className={cn(
                  "flex flex-col items-center rounded-xl border px-1 py-2 text-[0.65rem] font-bold transition-colors",
                  active
                    ? "border-se-primary bg-se-primary text-white"
                    : "border-se-outline-variant bg-white text-se-on-surface",
                )}
      ...
      <button
        onClick={() => {
          f.triggerGenerate();
          const palette = getTeamPalette(f.team);
          toast.success(
            `${palette.nameKo} ${f.dateStart} ~ ${f.dateEnd} 코스 추천 중...`,
            { description: "경기·길찾기·맛집 탭에서 결과를 확인하세요." },
          );
          const p = new URLSearchParams(searchParams.toString());
          p.set("team", f.team);
          p.set("start", f.dateStart);
          p.set("end", f.dateEnd);
          router.push(`/matches?${p.toString()}`);
          if (closeSheetOnGenerate) closeSheet();
        }}
        className="mb-3 w-full rounded-xl bg-[var(--se-signature-gradient)] bg-se-primary px-4 py-3 font-display text-sm font-extrabold text-white shadow-[0_6px_16px_rgba(0,25,60,0.2)] transition-transform hover:-translate-y-0.5"
      >
        🎯 코스 생성
      </button>
